...
def text_editor(id):
    ...
    try:
        return render_template("tools/text_editor.html", site=id, path=path, filename=filename, text=read_file(id, full_path).decode(), ver=VERSION, addons=updateAddons(), config=read_site_config(id)["Site"])
...
def test_tool(id):
    ...
    filename = request.args.get('filename', default="")
    full_path = f"{path}/{filename}"
    return render_template("tools/test_tool.html", site=id, path=path, filename=filename, ver=VERSION, addons=updateAddons(), config=read_site_config(id)["Site"])
